...
# (m_2=m_1*0.5) and Gaussian likelihoods.
################################################################################
...
for(m in 1:length(M)){
  ...
  if(M[m] <= 19998){
    ...
    results$time_estimation[i] <- system.time(lme4_model <- lmer(formula, REML = FALSE, data = data.frame(y = the_data$y, cbind(the_data$X, the_data$group_data)), 
  ...
  if(M[m] <= 9999){
    formula = as.formula(paste0("y ~ -1 + ",paste0(colnames(the_data$X), collapse = ' + ')," + ",
                                paste0("(1|",colnames(the_data$group_data),")", collapse = ' + ')))
    results$time_estimation[i] <- system.time(glmmTMB_model <- glmmTMB(formula, family=gaussian(), data=data.frame(y = the_data$y, cbind(the_data$X, the_data$group_data)),
                                                                       start=list(theta = init_cov_pars[-1], beta = init_betas)))[3]
  }

  i <- i + 1

  saveRDS(results, "./time_vs_m_unbalanced_Gaussian.rds")
  gc()
}
